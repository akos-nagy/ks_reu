...
Let $\Sigma$ be a smooth, closed surface with a Riemannian metric $g$ and area form $\omega$. Let $G$ be the Green operator of the Laplacian, $\Delta$. Let $I \subseteq \rl$ be a connected, open interval and $t_0 \in I$. For each function, $\varrho \in C^1 \left( I; L^1 \left( \Sigma \right) \right)$, let
\begin{equation}
	 \forall t \in I : \quad \varrho_t \eqdef \varrho|_{\{ t \} \times \Sigma} \in L^1 \left( \Sigma \right).
\end{equation}
Let us fix $\varrho_0 \in L^1 \left( \Sigma, g \right)$. A function, $\varrho \in C^1 \left( I; L^1 \left( \Sigma \right) \right)$, satisfies the \emph{(parabolic-elliptic) Keller--Segel equations} with initial value $\varrho_0$ if for all $t \in I$, $\varrho_t \in L_{1, \loc}^2$ and $\varrho$ is (weak) solution to the following system:
\begin{subequations}
\begin{align}
	\del_t \varrho								&= - \Delta \varrho + \rd^* \left( \varrho \rd G \left( \varrho \right) \right), \label{eq:KS_eq} \\
	\lim\limits_{t \rightarrow t_0^+} \varrho_t	&= \varrho_0. \label{eq:KS_IV}
\end{align}
\end{subequations}
...
\begin{lemma}
	Let $\chi_0 \eqdef \varrho_0 - \tfrac{M}{A_\Sigma} \in L^1 \left( \Sigma \right)$ and $\chi \eqdef \varrho - \tfrac{M}{A_\Sigma} \in C^1 \left( I; L^1 \left( \Sigma \right) \right)$. Then \cref{eq:KS_eq,eq:KS_IV} are equivalent to
	\begin{subequations}
	\begin{align}
		\del_t \chi									&= \tfrac{M}{A_\Sigma} \chi - \Delta \chi + \rd^* \left( \chi \rd G \left( \chi \right) \right), \label{eq:new_KS_eq} \\
		\lim\limits_{t \rightarrow t_0^+} \chi_t	&= \chi_0. \label{eq:new_KS_IV}
	\end{align}
	\end{subequations}
\end{lemma}
...
Since $\Sigma$ is compact, for all $p \in (1, \infty]$, we have that $L^p \left( \Sigma \right) \hookrightarrow L^1 \left( \Sigma, g \right)$. Let us assume now that $\varrho \in C^1 \left( I; L^2 \left( \Sigma \right) \right) \hookrightarrow C^1 \left( I; L^1 \left( \Sigma \right) \right)$ is a solution to the Keller--Segel \cref{eq:KS_eq}.
...
\begin{equation}
	\forall a \in \N : \ \forall t \in I : \quad R_a (t) \eqdef \left\langle \Psi_a \middle| \chi|_{\{ t \} \times \Sigma} \right\rangle_{L^2 \left( \Sigma, g \right)}.
\end{equation}
Then for each $a \in \N$, we have that $R_a \in C^1 \left( I; \rl \right)$. Note that $R_0 \equiv 0$. Finally let
\begin{equation}
...
\label{theorem:ODE}
	...
	\begin{subequations}
	\begin{align}
		\forall t \in I &: \quad \left( R_a (t) \right)_{a \in \N_+} \in l^2 \left( \N_+ \right), \label[cond]{cond:R_l2} \\
...
\begin{proof}
	Since $\left( \Psi_a \right)_{a \in \N}$ is an orthonormal basis of $L^2 \left( \Sigma, g \right)$ and for all $t \in I$, $\varrho_t$ is in $L^2 \left( \Sigma, g \right)$, we get \cref{cond:R_l2}.
...
Let $T > 0$ and assume that $I = (- T, T)$. In order to further simplify \cref{eq:R_a_eq,eq:S_a_eq}, we search for analytic solutions, that is
\begin{equation}
	\forall a \in \N_+ : \forall t \in I: \quad R_a (t) = \sum\limits_{n \in \N} R_{n, a} t^n, \label{eq:analycity}
\end{equation}
and the sums
\begin{equation}
	\forall t \in I: \sum\limits_{n \in \N} \left( R_{n, a} \right)_{a \in \N_+} t^n,
\end{equation}
are assumed to be absolute convergent in $l^2 \left( \N_+ \right)$. Similarly we define
\begin{equation}
	\forall a \in \N_+ : \forall t \in I: \quad S_a (t) = \sum\limits_{n \in \N} S_{n, a} t^n, \label{eq:analycity_for_S}
\end{equation}
...
\begin{lemma}
	...
	\end{align}
	\end{subequations}
	If $\exp \left( \Delta - \tfrac{M}{A_\Sigma} \id \right) \chi$ is also defined for all $t \in I$, then
	\begin{equation}
...
*?*add comment here about how showing short time existence is hard, but if we have it, then the next theorem is cool*

\begin{theorem}
	Let us assume that the coefficient $S_{n, a}$ exist for all $n \in \N$ and $a \in \N_+$. Moreover, assume that they are not all zero, and
	\begin{equation}
		\frac{1}{T} \eqdef \sup \left( \left\{ \ \sqrt[n]{S_{n, a}} \ \middle| \ n \in \N \mbox{ and } a \in \N_+ \ \right\} \right) < \infty.
	\end{equation}
\end{theorem}
...
The ideas and proofs of the previous sections still apply, and if $\chi \in C^1 \left( I, L_\cx^2 \left( \Sigma, g \right) \right)$, then we can define the coefficients functions $R_{\underline{k}}, S_{\underline{k}} \in C_\cx^1 \left( \Sigma \right)$, so that
\begin{align}
...
\begin{proof}
	...
	*This part is \TBC *
	...
	Now for all $t \in I$ and $\underline{x} \in \Sigma$, let
	\begin{equation}
