...
\begin{lemma}
	Let $\chi_0 \eqdef \varrho_0 - \tfrac{M}{A_\Sigma}$ and $\chi \eqdef \varrho - \tfrac{M}{A_\Sigma}$. Then \cref{eq:KS_eq,eq:KS_IV} is equivalent to
	\begin{subequations}
	\begin{align}
...
\begin{equation}
	\forall t \in (0, T): \quad R_a (t) \eqdef \left\langle \Psi_a \middle| \chi|_{\{ t \} \times \Sigma} \right\rangle_{L^2 \left( \Sigma, g \right)}.
\end{equation}
Then for each $a \in \N$, we have that $R_a \in C^1 \left( (0, T); \rl \right)$. Note that $R_0 \equiv 0$. Finally let
\begin{equation}
	\forall a, b, c \in \N : \quad \varphi_{abc} \eqdef \int\limits_\Sigma \Psi_a \Psi_b \Psi_c \dA.
\end{equation}
...
\label{theorem:ODE}
	...
	\begin{align}
		...
		\forall a \in \N_+ &: \quad \dot{R}_a = \left( \tfrac{M}{A_\Sigma} - \lambda_a \right) R_a + \sum\limits_{b, c \in \N_+} \frac{\lambda_a - \lambda_b + \lambda_c}{2 \lambda_c} \varphi_{abc} R_b R_c. \label{eq:R_a_eq}
	\end{align}
	\end{subequations}
...
\begin{equation}
	\forall a \in \N : \forall t \in (0, T): \quad R_a (t) = \sum\limits_{n \in \N} R_{n, a} t^n, \label{eq:analycity}
\end{equation}
and the right-hand side is assumed to be absolute convergent in $l^2 \left( \N \right)$.

The next lemma rewrites \cref{eq:R_a_eq} in terms of the coefficients $\left( R_{n, a} \right)_{(n, a) \in \N \times \N}$.

\begin{lemma}
	Under the above assumption, the function $\varrho$ is a $t$-analytic solution to \cref{eq:KS_eq} with mass $M$ exactly when $R_{0, 0} = \tfrac{M}{A_\Sigma}$, for all $n \in \N_+$, $R_{n, 0} = 0$, and
	\begin{subequations}
	\begin{align}
		\forall a, n \in \N	&: \quad R_{n + 1, a} = \frac{1}{n + 1} \left( \left( \tfrac{M}{A_\Sigma} - \lambda_a \right) R_{n, a} + \sum\limits_{b, c \in \N_+} \sum\limits_{m = 0}^n \tfrac{\lambda_a - \lambda_b + \lambda_c}{2 \lambda_c} \varphi_{abc} R_{m, b} R_{n - m, c} \right), \label{eq:general_iteration} \\
		\forall n \in \N	&: \quad \cR_n \eqdef \left( R_{n, a} \right)_{a \in \N} \in l^2 \left( \N \right), \label{eq:regularity} \\
		& \qquad \limsup\limits_{n \rightarrow \infty} \| \cR_n \|_{l^2 \left( \N \right)}^{\frac{1}{n}} = \limsup\limits_{n \rightarrow \infty} \left( \sum\limits_{a \in \N} R_{n, a}^2 \right)^{\frac{1}{n}} < \frac{1}{T}. \label{eq:convergence_radius}
	\end{align}
	\end{subequations}
\end{lemma}
...
\begin{equation}
	R_{n + 1, \underline{k}} = \frac{1}{n + 1} \left( \left( \tfrac{M}{A_\Sigma} - 4 \pi^2 \left| \underline{k} \right|^2 \right) R_{n, \underline{k}} + \sum\limits_{\underline{l} \in \Lambda^*} \sum\limits_{m = 0}^n \frac{\underline{k} \cdot \underline{l}}{\left| \underline{l} \right|^2} R_{m, \underline{l}} R_{n - m, \underline{k} - \underline{l}} \right). \label{eq:iteration_on_torus}
\end{equation}
...
\begin{theorem}
	...
	\begin{equation}
		R_{n + 1, \underline{k}} = \frac{1}{n + 1} \left( \left( \tfrac{M}{A_\Sigma} - 4 \pi^2 \left| \underline{k} \right|^2 \right) R_{n, \underline{k}} + \sum\limits_{\substack{\underline{l} \in Z_m \\ \underline{k} - \underline{l} \in Z_{n - m}}} \sum\limits_{m = 0}^n \frac{\underline{k} \cdot \underline{l}}{\left| \underline{l} \right|^2} R_{m, \underline{l}} R_{n - m, \underline{k} - \underline{l}} \right). \label{eq:iteration_on_torus_finite}
	\end{equation}
	Finally, let
	\begin{equation}
		d_n \eqdef \max \left( \left\{ \ \left| \underline{k} \right| \ \middle| \ \underline{k} \in Z_n \ \right\} \right) > 0.
	\end{equation}
	Then
	\begin{equation}
		d_n \leqslant (n + 1) d_0. \label[ineq]{ineq:d_n}
	\end{equation}
\end{theorem}
...
\begin{proof}
	...
	Now let $Z_{n + 1}^\prime \eqdef Z_n + Z_n$. If $\underline{k} \in \Lambda^* - Z_{n + 1}^\prime$, then for all $m \in \N$, such that $0 \leqslant m \leqslant n$, there does not exists $\underline{l} \in Z_m$ that could also satisfies $\underline{k} - \underline{l} \in Z_{n - m}$. Furthermore, since $Z_n \subseteq Z_{n + 1}^\prime$ we also have that $\underline{k} \notin Z_n$. Thus all terms in \cref{eq:iteration_on_torus_finite} are zero, and hence $\underline{k} \in Z_{n + 1}$, which proves \cref{eq:Z_n_iteration}.

	Finally, if $\underline{k} \in Z_{n + 1}$, then there exist $m \in [0, n] \cap \N$, $\underline{l}_1 \in Z_m$, and $\underline{l}_2 \in Z_{n - m}$, such that $\underline{k} = \underline{l}_1 + \underline{l}_2$. Thus
	\begin{equation}
		\left| \underline{k} \right|  = \left| \underline{l}_1 + \underline{l}_2 \right| \leqslant \left| \underline{l}_1 \right| + \left| \underline{l}_2 \right| \leqslant d_m + d_{n - m}.
	\end{equation}
	Once again, using induction and the fact that \cref{ineq:d_n} holds trivially for $n = 0$, we get that if \cref{ineq:d_n} holds for all nonnegative integers up to and including $n$. Then if $\underline{k}$ is an element such that $\left| \underline{k} \right| = d_{n + 1}$, then
	\begin{align}
		d_{n + 1} = \left| \underline{k} \right| \leqslant d_m + d_{n - m} \leqslant (m + 1) d_0 + (n - m + 1) d_0 = \left( (n + 1) + 1 \right) d_0,
	\end{align}
...
\begin{theorem}
	Under the assumptions of \Cref{theorem:torus_finite}, we have the following: There exists $C > 0$, that only depends on $\cR_0$, as in \cref{eq:regularity}, and the geometry of $\Sigma$, such that for all $n \in \N$ and $\underline{k} \in \Lambda^*$ we have
	\begin{equation}
		\left| R_{n, \underline{k}} \right| \leqslant C^{n + 1} \exp \left( - \tfrac{\left| \underline{k} \right|}{2^n C} \right).
	\end{equation}
\end{theorem}
...
\begin{proof}
	Let
	\begin{equation}
		C^\prime \eqdef \inf \left( \left\{ \ c \in \rl_+ \ \middle| \ \forall \underline{k} \in Z_0 - \{ \underline{0} \} : \quad \left| R_{0, \underline{k}} \right| \leqslant c \exp \left( - \tfrac{\left| \underline{k} \right|}{c} \right) \ \right\} \right),
	\end{equation}
	and let
	\begin{equation}
		C \eqdef \max \left( \left\{ \ C^\prime, ??? \ \right\} \right).
	\end{equation}
	We prove that $C$ is the constant in the claim using induction. By construction the claim is true for $n = 0$. Let us assume that we have already prove the claim for all nonnegative integers up to and including $n \in \N_+$.

	Let $\underline{k} \in Z_{n + 1}$. Then we have
	\begin{align}
		\left| \frac{1}{n + 1} \left( \tfrac{M}{A_\Sigma} - 4 \pi^2 \left| \underline{k} \right|^2 \right) R_{n, \underline{k}} \right|	&= \frac{1}{n + 1} C \left| \underline{k} \right|^2 C^{n + 1} \exp \left( - \tfrac{\left| \underline{k} \right|}{2^n C} \right)
	\end{align}
