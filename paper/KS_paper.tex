...
\begin{align}
	\Psi_{(l, m)}							&\eqdef \Psi_l^m, \\
	R_{l, a}^m 								&\eqdef R_{(l, m), a}, \\
	\varphi_{l_1, l_2, l_3}^{m_1, m_2, m_3}	&\eqdef \varphi_{(l_1, m_1), (l_2, m_2), (l_3, m_3)}.
\end{align}
Using this new set of indices and notation, we can rewrite \cref{eq:general_iteration} as
\begin{align}
	\forall l \in \N_+: \quad R_{l, n + 1}^m	&= \frac{1}{n + 1} \left( \tfrac{M}{4 \pi r^2} - \tfrac{l (l + 1)}{r^2} \right) R_{(l, m), n} \\
...
\begin{proposition}
	...
	\begin{align}
		R_{l, n + 1}^m	&= \frac{1}{n + 1} \left( \tfrac{M}{4 \pi r^2} - \tfrac{l (l + 1)}{r^2} \right) R_{l, n}^m \\
					& \quad + \sum\limits_{(l_1, m_1) \in Z_n} \sum\limits_{l_2 = \max \left( \{ 1, |l - l_1| \} \right)}^{l + l_1} \sum\limits_{m_2 = - l_2}^{l_2} \sum\limits_{x = 0}^n \tfrac{l (l + 1) - l_1 (l_1 + 1) + l_2 (l_2 + 1)}{2 (n + 1) l_2 (l_2 + 1)} \varphi_{l, l_1, l_2}^{m, m_1, m_2} R_{l_1, x}^{m_1} R_{l_2, n - x}^{m_2}.
	\end{align}
\end{proposition}
